...
Proof.
  ...
  - exists x. split; first by exists 0.
    ...
    clear -HM. rewrite /step' /step.
    case Hi: (nth_error M (state x)) => [i|] H; last done.
  ...
  - (* case: (1, 1) f->>t-> (S a', 0) loop or uniform transition to (0, 0) *)
    ...
    + (* uniform transition to (0, 0) *)
      ...
      { clear -HM. move=> H a. elim: (S a); first by exists 0.
        move=> {}a IH. by apply: (reaches_trans (H a) IH). }
      move=> {}a.
      move: Hx' => /reaches'E [n] [m] /= [?] [?].
      move=> /(_ (S a) 0 ltac:(lia) ltac:(lia)) /reaches'_incl.
      move=> /reaches_trans. apply.
      move: H'x' => /(step_parallel (state x', (S a + n, m))) /=.
      move=> /(_ ltac:(lia)) [[pz [az bz]]] [Hz] /= [? ?].
      move: Hz => /step_reaches /reaches_trans. apply.
      move: (H az bz). congr reaches; congr (_, (_, _)); lia.
    + (* non-termination *)
      do 3 left. right. move=> a. apply: transition_loop => a' b' ??.
      move: Hx' H'x' => /reaches'E [n] [m] /= [?] [?].
      move=> /(_ a' b' ltac:(lia) ltac:(lia)) /reaches'_incl Hk'.
      move=> /(step_parallel (state x', (a' + n, b' + m))) /=.
      move=> /(_ ltac:(lia)) [[pz [az bz]]] /= [/step_reaches_plus Hz] [? ?]. subst pz.
      exists (bz * S a'y + az), 0. split; last by lia.
...
Proof.
  ...
  { (* case: (0, 1) f->>t-> (0, _) *)
    ...
    - (* case: (0, 1) f->>t-> (0, 0) uniform transition to (0, 0) *)
      do 2 left. right. move=> b. apply: dec_b_0'; by eassumption.
    - (* non-termination *)
  ...
  - (* case: (1, 1) f->>t-> (0, S b') loop or uniform transition to (0, 0) *)
    ...
    + (* uniform transition to (0, 0) *)
      do 2 left. right.
      suff: (forall b, reaches (0, (0, S b)) (0, (0, b))).
      { clear -HM. move=> H b. elim: (S b); first by exists 0.
        move=> {}b IH. by apply: (reaches_trans (H b) IH). }
      move=> {}b.
      move: Hx' => /reaches'E [n] [m] /= [?] [?].
      move=> /(_ 0 (S b) ltac:(lia) ltac:(lia)) /reaches'_incl.
      move=> /reaches_trans. apply.
    ...
    + (* non-termination *)
      ...
      move=> /(step_parallel (state x', (a' + n, b' + m))) /=.
      move=> /(_ ltac:(lia)) [[pz [az bz]]] /= [/step_reaches_plus Hz] [? ?]. subst pz.
      exists 0, (az * S b'y + bz). split; last by lia.
      by apply /(reaches_reaches_plus Hk') /(reaches_plus_reaches Hz) /H.
